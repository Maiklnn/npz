class Scroll {
	...
	scroll() {
		...
		if(this.element) {
			...
			*/
			let scrollElem = this.target.dataset.scroll.split('--');
			let scrollBlock = document.querySelector(`${scrollElem[0]}`); // получаем блок до которого скролим
			...
			if(scrollBlock.classList.contains('tab__nav')) {
				let tabLinks = document.querySelectorAll('[data-tab_link]')
				let tabContent = document.querySelectorAll('[data-tab_content]')
...
class SliderSwiper {
	...
	sliders() {
		...
		sliders = document.querySelectorAll('.slider-cat__swiper');
		sliders.forEach((item) => {
			new Swiper(item, {
				spaceBetween: 34,
				autoplay: { delay: 3000 },
...
class Range {
	...
	range() {
		...
		function range(item) {
			let dataRange = item.dataset.range.split(';');
			let min;
			let max;
			let startInit;
			let endInit;
			dataRange.forEach(item =>{
				if(item.includes('min=')) min = +item.replace('min=','');
				if(item.includes('max=')) max = +item.replace('max=','')
				if(item.includes('startInit=')) startInit = +item.replace('startInit=','')
				if(item.includes('endInit=')) endInit = +item.replace('endInit=','')
			});
			noUiSlider.create(item, {
				start: [startInit, endInit],
				connect: true,
				range: {
					'min': [min],
					'max': [max]
			...
			const input0 = item.parentElement.querySelector("[data-input='1']");
			const input1 = item.parentElement.querySelector("[data-input='2']");
			const inputs = [input0, input1];

			item.noUiSlider.on('update', function(values, handle){
				let num = +values[handle]
				inputs[handle].value = num.toFixed(1);
			});
		...
		if (rangeSlider) {
			rangeSlider.forEach(item => range(item));
		}
...
class Slider_thumbnail {
	...
	constructor() {
		...
		document.addEventListener('click', e => {
			this.e = e
			this.target = e.target
			this.slide = this.target.closest('[data-index]')
			if(this.slide) this.slideSelected()
		});
	...
	init() {
		...
		this.widthShift = this.wrapper.offsetWidth / this.slideCount - this.slideOffset


		this.sliderItems = this.slider.querySelectorAll('.slide')

		this.sliderItems.forEach((item, index) => {
			if(index === this.slideCurrent) item.dataset.current = 1
			item.dataset.index = index
			item.style.minWidth = `100px`;
			item.style.marginRight = `${this.slideOffset}px`;
		})

		this.dataSlider = this.slider.dataset.slider

		if(this.dataSlider !== 1) {
			this.loadContainer = this.slider.parentElement.querySelector(`${this.slider.dataset.slider}`)
			this.dataIndex = this.slider.querySelector(`[data-index="${this.slideCurrent}"]`)


			this.loadContainer.innerHTML = this.dataIndex.outerHTML;
		}
	...
	slideSelected() {
		...
		if(this.slide.dataset.index === 'next') {
			this.dataCurrent = this.slider.querySelector('[data-current]')
			delete this.dataCurrent.dataset.current

			if(this.dataCurrent) {
				if(!this.dataCurrent.dataset.index) this.dataCurrent = this.dataCurrent.parentElement


				this.dataCurrent = +this.dataCurrent.dataset.index + 1
				if((this.sliderItems.length - 1) < this.dataCurrent) this.dataCurrent = 0
			} else {
				this.dataCurrent = 0
			}


			this.nextSlide = this.slider.querySelector(`[data-index="${this.dataCurrent}"]`)
			this.nextSlide.dataset.current = this.dataCurrent

			// сдвиг
			this.widthShift = ((this.wrapper.offsetWidth + this.slideOffset) / this.slideCount) * this.dataCurrent
			console.log(this.wrapper.offsetWidth / this.slideCount)
			this.wrapper.style.transform = `translateX(-${this.widthShift}px)`;
			this.loadContent = this.nextSlide.outerHTML
		}

		if(this.slide.dataset.index !== 'next' && this.slide.dataset.index !== 'last') {
			let current = this.slider.querySelector('[data-current]')
			delete current.dataset.current
			this.loadContent = this.target.outerHTML
			this.target.dataset.current = 1
		}

		if(this.slide.dataset.index === 'last') {
			this.dataCurrent = this.slider.querySelector('[data-current]')
			delete this.dataCurrent.dataset.current

			if(this.dataCurrent && this.dataCurrent.dataset.index !== '0') {
				if(!this.dataCurrent.dataset.index) this.dataCurrent = this.dataCurrent.parentElement
				this.dataCurrent = +this.dataCurrent.dataset.index - 1
			} else {
				this.dataCurrent = this.sliderItems.length - 1
			}
		...
		this.loadContainer.innerHTML = this.loadContent;
		let img = this.loadContainer.querySelector('img')
		img.dataset.popupLoad = 'current'
	}

}

new Slider_thumbnail();
...
document.addEventListener('click', e => {
	...
	*/
	...
	{
		e.preventDefault()
		let dataShow = clickedElement.dataset.show
		let yesData = dataShow.includes(':');
		...
		if(yesData) {
			...
			if(data[0] === 'all') {
				if(data[1] === 'parent') {
					let parent = target.closest(`[data-is_show]`)
					console.log(parent)
					let is_show = parent.querySelectorAll(`[data-is_show="sub"]`)

					// отключаем активные
					let menu = document.querySelector(`.cat-menu`)
					let close_show = menu.querySelectorAll(`[data-is_show="sub"]`)
					close_show.forEach((item) => {
						item.classList.remove('show')
					});

					// добовляем класс show к выбранным
					is_show.forEach((item) => {
						item.classList.toggle('show')
					});
				}

				let is_show = document.querySelectorAll(`[data-is-show="${data[1]}"]`)
				is_show.forEach((item) => {
					item.classList.toggle('show')
				});
				return
